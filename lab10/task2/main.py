...
SW, SH = 600, 600 # screen (playing area)
...
class Snake:
    def __init__(self):
        ...
        self.xdir = 1
        self.ydir = 0
        self.head = pygame.Rect(self.x, self.y, BLOCK_SIZE, BLOCK_SIZE) # rectangle head
        self.body = [pygame.Rect(self.x-BLOCK_SIZE, self.y, BLOCK_SIZE, BLOCK_SIZE)] #body rectangles
        self.dead = False #indicating snake is dead
        self.restart = False #indicating restart key pressed

    def update(self):
        global apple, wall, golden_apple

        for square in self.body:
            if self.head.x == square.x and self.head.y == square.y: #col head with its body
                self.dead = True
            if self.head.x not in range(0, SW) or self.head.y not in range(0, SH): #if the snake leaves the area
                self.dead = True
        # snake dead, restarrt true
        if self.dead and self.restart:
                ...
                wall = Wall() #  new wall
                golden_apple = GoldenApple(self.body, (apple.x, apple.y), [barrier for barrier in wall.barriers]) # new gold
...
class Apple:
    ...
    # updating the position and drawing the apple
...
class GoldenApple:
    def __init__(self, snake_body, apple_pos, wall_barriers):
        # init spawn time and rect
        self.spawn_time = pygame.time.get_ticks()
        self.golden_apple_rect = None
        if random.random() <= 0.1: # call spawn_golden_apple if <=0.1 10%
        ...
        while True:
            ...
            if (self.x, self.y) not in apple_pos and \
               ...
               (self.x, self.y) not in [(barrier.x, barrier.y) for barrier in wall_barriers]:
                ...
                break
    #update position and appearence 
    def update(self, snake_body, apple_pos, wall_barriers):
        current_time = pygame.time.get_ticks()

        if self.golden_apple_rect is not None: # golden apple exists 
            if current_time - self.spawn_time >= 3000: #despawn it (3 seconds)
                self.golden_apple_rect = None  
        else: # if doesnt exist, if nessess to create new one 
...
class Wall:
    ...
    def update(self, snake_body, apple_pos, snake_head_pos, eaten_fruits):
        ...
        eaten_fruits = eaten_fruits // 2 #new wall every 2 food
...
while not done:
    ...
    pygame.draw.rect(screen, (0, 255, 0), snake.head) #snake's head
    ...
    if snake.head.x == apple.x and snake.head.y == apple.y:
        ...
        eaten_fruits += 1
        score += 1
        if (len(snake.body)-1) % 5 == 0: #  speed is inc after every fifth fruit
            speed += 0.5

    # new apple will not be created on an existing wall
